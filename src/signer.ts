...
function generateTypeHash(contract: Contract, method: string): string {
  ...
  for (let i = 0; i < fragment.inputs.length; i++) {
    let { name, type } = fragment.inputs[i];
...
export async function sign (provider: any, arcana: Contract, forwarder: Contract, method: any, params: any): Promise<ContractTransaction> {
  ...
  request.data = localEncode(arcana,params,method);
